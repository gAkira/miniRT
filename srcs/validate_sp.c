...
/*   Updated: 2021/03/24 18:52:56 by galves-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	valid_args_range(char **tab)
{
	...
	if (!in_rangef(ft_atof(first[0]), -DBL_MAX, DBL_MAX) || \
		...
		!in_rangef(ft_atof(second[0]), 0.0, DBL_MAX) || \
		!in_range(ft_atoi(third[0]), 0, 255) || \
...
**    validate_sp
** Description:
**    Validate each sphere's (sp) arguments and parameters
...
**                  (i.e. a different sphere) of the file. At the second
**                  index (sp[index1][index2]) is a word (string) of the
**                  respective line
** Return:
**    t_error -> report the error found
**       [NO_ERROR] :: everything is alright
**       [WRONG_NUM_ARGS_SP] :: wrong number of arguments passed
**       [BAD_FORMAT_SP] :: some argument has a wrong number of parameters
**       [WRONG_ARGS_RANGE_SP] :: parameter of some argument is out of range
*/
