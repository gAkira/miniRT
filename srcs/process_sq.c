...
/*   Updated: 2021/04/01 21:45:27 by galves-d         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	put_in_scene(char ***sq, char ***params, t_scene *scene, int i)
{
	...
	scene->sq[i]->mat.color = mx_vector(ft_atof(params[2][0]) / 255.0, \
								ft_atof(params[2][1]) / 255.0, \
								ft_atof(params[2][2]) / 255.0);
	scene->sq[i]->mat.ambient = 0.1;
	scene->sq[i]->mat.diffuse = 0.9;
	scene->sq[i]->mat.specular = 0.9;
	scene->sq[i]->mat.shininess = 200.0;
}
